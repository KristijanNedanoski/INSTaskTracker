...
namespace INSTaskTracker.Logic
{
    internal class RoleActions
    {
        ...
        internal void CreateClient()
        {
            ...
            // When creating the RoleManager object, you pass in (as a parameter) a new RoleStore object.
            var roleMgr = new RoleManager<IdentityRole>(roleStore);
            // Then, you create the "Client" role if it doesn't already exist.
            if (!roleMgr.RoleExists("Client"))
            {
                IdRoleResult = roleMgr.Create(new IdentityRole("Client"));
                if (!IdRoleResult.Succeeded)
                {
                    // Handle the error condition if there's a problem creating the RoleManager object.
                }
        ...
        internal void CreateDeveloper()
        {
            ...
            // When creating the RoleManager object, you pass in (as a parameter) a new RoleStore object.
            var roleMgr = new RoleManager<IdentityRole>(roleStore);
            // Then, you create the "Developer" role if it doesn't already exist.
            if (!roleMgr.RoleExists("Developer"))
            {
                IdRoleResult = roleMgr.Create(new IdentityRole("Developer"));
                if (!IdRoleResult.Succeeded)
                {
                    // Handle the error condition if there's a problem creating the RoleManager object.
                }
